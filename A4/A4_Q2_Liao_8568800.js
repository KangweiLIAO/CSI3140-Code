	...
	this.onclick = (e) => {
		...
		if (checkWinning()) {
			...
			setTimeout(() => {
				if (
					window.confirm(
						"Congratulation! You win! Do u want to play again?"
					)
				) {
					// if play again
...
/**
 * Check the wining condition is true or not
 * @returns true if the table is in order, false otherwise
 */
function checkWinning() {
	...
	Array.from(tbody.children).forEach((row) => {
		Array.from(row.children).forEach((cell) => {
			if (counter != cell.textContent && cell.textContent != "") {
				// console.log(`${counter} = ${cell.textContent}`);
				flag = false;
			}
			counter++;
		});
